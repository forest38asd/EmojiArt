...
extension UTType {
    static let emojiart = UTType(exportedAs: "gross.emojiart")
}
...
class EmojiArtDocument: ReferenceFileDocument {
    ...
    func moveEmoji(_ emoji: EmojiArtModel.Emoji, by ofset: CGSize, undoManager: UndoManager?) {
        if let index = emojiArt.emojis.index(matching: emoji) {
            undoablyPerform(operation: "Move", with: undoManager) {
                emojiArt.emojis[index].x += Int(ofset.width)
                emojiArt.emojis[index].y += Int(ofset.height)
            }
        }
    }
